            ...
            if (mappings && mappings.length > 0) {
              ...
              tagbody.replace(this.attr, function (attr, key, value, idx) {
                ...
                if (key === map && map.conf.where || 'id' && data[value]) {
                  ...
                  var output  = '',
                      segment = html.slice(
                        html.search(components[0]) + components[0].length, 
                        html.lastIndexOf(tagname) - 2
                      );
                  ...
                  // Is it an object?
                    ...
                    // Yes : bind the html segment to the data value
                    
                    // Account for the handing markup from the parent
                    //segment = segment.slice(0, segment.lastIndexOf(tagname) -2);
                    output += that.bind(segment, data[value], map);
