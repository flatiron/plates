...
;!function(undefined) {
  ...
  var splice = function splice(s1, idx, s2) {
    return (s1.slice(0, idx) + s2 + s1.slice(idx));
  }
  ...
  Plates.prototype.bind = function(map) {
    ...
    for (var d in this._data) {
      //if (this._data.hasOwnProperty(d)) {
        ...
        //
        if(this._map) {
          elems = this._tree.map[this._map[d]][d];
        }
        else {
          elems = this._tree.map['id'][d];
        }
        ...
        for(var i=0, l=elems.length; i < l; i++) {
          that._out = splice(that._out, elems[i].body[1], that._data[d]);
          invoked = true;
        }
