...
;!function(undefined) {
  ...
  function attSup(all, name, value1, value2) {
  ...
  Plates.prototype.bind = function(map) {
    ...
    for (var d in this._data) {
      if (this._data.hasOwnProperty(d)) {
        //
        // if the user wants to be explicit abou the
        // mapping of the attributes and the data.
        //
        if (this._map[d]) {
          ...
          //
          elems = this.tree.map[this._map[d] || 'id'][d];

          if (!elems) {
            elems = this.tree.map['data-bind'][d];
          }
          ...
          elems && elems.forEach(function(elem) {
            splice(this._out, elem.body[1], this._data[d]);
          });
        }
      }
    }
