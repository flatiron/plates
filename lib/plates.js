  ...
  Merge.prototype = {
    ...
    iterate: function(html, value, components, tagname, key) {
      ...
      if (isArray(value)) {
        ...
        // not required
        return output = output.slice(0, -(tagname.length + 3));
        ...
        if (c === '!' && intag && !matchmode) {
          ...
          if (attributes && !isClosing && !matchmode) {
            ...
            if (mappings && mappings.length > 0) {
              ...
              for (var ii = mappings.length - 1; mappings[ii]; ii--) {
                ...
                tagbody = tagbody.replace(this.attr, function(str, key, value, a) {
                  ...
                  if (shouldSetAttribute && mappings[ii].replace !== key) {
                    ...
                    if (mappings[ii].value === value || that.hasClass(value, mappings[ii].value || mappings.conf.where === key)) {
                      ...
                      newdata = (isArray(newdata) || typeof newdata === 'object') ? that.iterate(html, newdata, components, tagname, value) : newdata;
              ...
              tagbody.replace(this.attr, function (attr, key, value, idx) {
                ...
                if (key === map && map.conf.where || 'id' && data[value]) {
                  ...
                  var v      = data[value],
                      output = (isArray(v) || typeof v === 'object') ? that.iterate(html, v, components, tagname, value) : v;

                  buffer += isArray(v) ? output : tagbody + output;
                  matchmode = true;
                } 
