  ...
  Merge.prototype = {
    ...
    bind: function bind(html, data, map) {
      ...
      var openers = 0,
          ...
          ignoreEmptyAttributes = map && map.conf && map.conf.ignoreEmpty,
          closing,
          tagbody;
              ...
              for (var ii = mappings.length - 1; ii >= 0; ii--) {
                ...
                if (createAttribute && shouldSetAttribute && !setAttribute) {
                  var spliced = selfClosing ? 2 : 1
                    ...
                    , attributeValue = fetch(data, mapping)
                    , ignore = false;

                  //
                  // Can this attribute be ignored?
                  //
                  if (typeof(attributeValue) == 'undefined') {
                    switch (typeof(ignoreEmptyAttributes)){
                      case 'object':
                        if (Array.isArray(ignoreEmptyAttributes)) {
                          ignore = ignoreEmptyAttributes.indexOf(mapping.replace) >= 0;
                        }
                        break;
                      case 'string':
                        ignore = ignoreEmptyAttributes === mapping.replace;
                        break;
                      case 'boolean':
                        ignore = ignoreEmptyAttributes;
                        break;
                    }
                  }
